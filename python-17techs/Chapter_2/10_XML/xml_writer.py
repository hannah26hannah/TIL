...
def to_xml(tree, dict_object):
    for key in dict_object:
        element = etree.SubElement(tree, key)  # key에 해당하는 sub tree 객체를 생성
        value = dict_object[key]
    if key:
        # 키에 대한 값이 존재하는 경우 값의 타입을 확인 후 처리
        if type(value) is str:
            # dict 값이 단순 문자열인 경우 값만 추가
            element.text = value
...
with open('message2.xml', 'wb') as file:
    file.write(etree.tostring(
        xml_tree, xml_declaration=True, encoding='UTF8', pretty_print=True
    ))

# result -> message2.xml str_array 값만 제대로 생성됨.
